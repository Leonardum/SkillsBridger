...
function loadEvents(studentId) {
	...
	xmlhttp.onreadystatechange = function () {
		if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
			...
			if (arrayLength == 0) {
				eventList.innerHTML = "<p class='darkWatermark'>It looks like you are not subscribed to any event.</p>";
			} else {
				for (i = 0; i < arrayLength; i++) {
					...
					input3.setAttribute("value", "subscribedEvents");
					...
					unsubscribeButton.setAttribute("class", "unsubscribe-button");
					unsubscribeButton.setAttribute("type", "button");
					unsubscribeButton.setAttribute("onclick", "unsubscribeConfirm('" + x + "');");
					unsubscribeButton.innerHTML = "Unsubscribe from this event!";
					form.appendChild(unsubscribeButton);
	...
	xmlhttp.open("GET", "./model/ajax/eventListFetch.php?subscribed=1&studentId=" + studentId, true);
...
function loadAdditionalInfo(ids) {
	'use strict';
	/* ids is an array of the shape = [[1,1,1,1], [2,2,1,1]] and ids[i] = [i+1, eventId, organisationId, addressId] */
