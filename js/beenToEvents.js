...
function loadEvents(studentId) {
	...
	xmlhttp.onreadystatechange = function () {
		if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
			...
			if (arrayLength == 0) {
				eventList.innerHTML = "<p class='darkWatermark'>It looks like you haven't been to any events yet!</p>";
			} else {
				for (i = 0; i < arrayLength; i++) {
					...
					ids.push([x, events[i].Event_id, events[i].Organisation_id, events[i].Address_id, AUXLIB.studentId()]);
					
					var link = document.createElement('a');
                    link.setAttribute("href", "index.php?action=studentEventOverview&eventId=" + events[i].Event_id);
					...
					ratingRow.setAttribute("id", "rating" + x);
					ratingRow.setAttribute("class", "col-20 ratingRow");
					eventBox.appendChild(ratingRow);
	...
	xmlhttp.open("GET", "./model/ajax/eventListFetch.php?attended=1&studentId=" + studentId, true);
    ...
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
			...
			for (i = 0; i < arrayLength; i++) {
				...
				// Add the rating form or show the rating of the student.
				if (skills != 0) {
					var ratingRow = document.getElementById('rating' + x);
					
					var ratingPar = document.createElement('p');
					ratingPar.innerHTML = "Did this event offer the skills stated above<br>(1 = not at all, 5 = absolutely) ?";
					ratingRow.appendChild(ratingPar);
					
					if (rating == 0) {
						var form = document.createElement('form');
						form.setAttribute("action", AUXLIB.thisPage());
						form.setAttribute("method", "post");
						ratingRow.appendChild(form);
						
						var ratingSelect = document.createElement('select');
						var ratingNoSelection = document.createElement('option');
						ratingNoSelection.selected = true;
						ratingNoSelection.disabled = true;
						ratingNoSelection.hidden = true;
						ratingNoSelection.setAttribute("value", "");
						ratingSelect.appendChild(ratingNoSelection);
						ratingSelect.setAttribute("name", "rating");
						var j;
						for (j = 1; j < 6; j++) {
							var ratingOption = document.createElement('option');
							ratingOption.value = j;
							ratingOption.text = j;
						...
						input3.setAttribute("value", "beenToEvents");
						form.appendChild(input3);
						var rate = document.createElement('input');
						rate.setAttribute("type", "submit");
						rate.setAttribute("name", "submit");
						rate.setAttribute("value", "Rate this event!");
						form.appendChild(rate);
					} else {
						var rated = document.createElement('p');
						rated.setAttribute("style", "margin: 5px 0;");
						rated.innerHTML = "You gave this event a rating of: " + rating;
						ratingRow.appendChild(rated);
					}
				}
			}
        }
    }
    xmlhttp.open("GET", "./model/ajax/additionalEventInfoFetch.php?ratingNeeded=1&ids=" +  JSON.stringify(ids), true);
    xmlhttp.send();
}
