...
function loadOrganisationEvents(organisationId) {
    ...
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
            ...
            if (arrayLength == 0) {
                eventList.innerHTML = "<p class='darkWatermark'>There are no events your organisation has hosted in our database yet.</p>";
            } else {
                for (i = 0; i < arrayLength; i++) {
                    ...
                    link.setAttribute("href", "index.php?action=eventManager&eventId=" + events[i].Event_id + "&organisationId=" + organisationId + "&senderPage=passedEvents");
    ...
    xmlhttp.open("GET", "./model/ajax/passedEventsFetch.php?organisationId=" + organisationId, true);
    xmlhttp.send();
}
...
function loadAdditionalInfo(ids) {
    ...
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
			...
			for (i = 0; i < arrayLength; i++) {
				...
				if (skills == 0) {
					ratingRow.setAttribute("style", "border-top:1px solid rgba(255, 255, 255, 0.3);");
					ratingPar.innerHTML = "Events offering no skills will not be rated.";
				} else if (rating === 0) {
					ratingRow.setAttribute("style", "border-top:1px solid rgba(255, 255, 255, 0.3);");
					ratingPar.innerHTML = "Your event has not been rated yet!";
				} else {
					ratingPar.innerHTML = "Your event has been rated: " + rating + "!";
				}
    ...
    xmlhttp.open("GET", "./model/ajax/additionalEventInfoFetch.php?getRating=1&ids=" +  JSON.stringify(ids), true);
    xmlhttp.send();
}