var careerGoalSkills;
if (AUXLIB.passedCareerGoals() != undefined) {
	var selectedCareerGoals = AUXLIB.passedCareerGoals().split(',');
	var k;
	for (k = 0; k < selectedCareerGoals.length; k++) {
		selectedCareerGoals[k] = parseInt(selectedCareerGoals[k]);
	}
} else {
	var selectedCareerGoals = [];
}
var skillsToDisplay;
...
function loadCareerGoals() {
	...
	var careerGoalsRequest = new XMLHttpRequest();
    careerGoalsRequest.onreadystatechange = function () {
        if (careerGoalsRequest.readyState === 4 && careerGoalsRequest.status === 200) {
			careerGoalSkills = JSON.parse(careerGoalsRequest.responseText);
        }
    }
    careerGoalsRequest.open("GET", "./model/ajax/learningEventsFetch.php?skillsMapLoad=true", true);
    careerGoalsRequest.send();
	
	
    var careerGoalsDiv = document.getElementById('careerGoals');
    
    /* Use AJAX to get the array of level 1 career goals from the database and to create the career goal checkboxes with. */
    ...
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
			...
			careerGoalsDiv.appendChild(instruction);
			
			var instructionTitle = document.createElement('p');
            instructionTitle.setAttribute("class", "instructionTitle");
			instructionTitle.innerHTML = "By career goal:";
			careerGoalsDiv.appendChild(instructionTitle);
            ...
            for (i = 0; i < arrayLength; i++) {
                ...
                var parentId = careerGoals[i][1];
                
                var checkBox = document.createElement('input');
                checkBox.setAttribute("type", "checkbox");
                checkBox.setAttribute("id", parent.toLowerCase().replace(/\s+/g, '') + "Box");
				if (typeof AUXLIB.checkList() !== 'undefined') {
					if (partOfArray(parentId, AUXLIB.checkList())) {
						checkBox.checked = true;
					}
				}
				checkBox.setAttribute("onchange", "processSelection(this, 'careerGoal', " + parentId + ")");
                form.appendChild(checkBox);
                
                var span = document.createElement('span');
                span.setAttribute("class", "tickBoxText");
                span.innerHTML = parent;
			...
			var checkedCareergoals = document.createElement('input');
            checkedCareergoals.setAttribute("type", "hidden");
            checkedCareergoals.setAttribute("id", "checkedCareergoals");
            checkedCareergoals.setAttribute("name", "checkedCareergoals");
			if (typeof AUXLIB.passedCareerGoals() !== 'undefined') {
            	checkedCareergoals.setAttribute("value", AUXLIB.passedCareerGoals());
			} else {
				checkedCareergoals.setAttribute("value", "");
			}
            form.appendChild(checkedCareergoals);
...
function loadRegions() {
    ...
    regionsRequest.open("GET", "./model/ajax/learningEventsFetch.php?regionsLoad=true", true);
    regionsRequest.send();
}


/* This function creates and loads all upcoming event information and prepares a display layout for them. */
function loadEvents() {
    'use strict';
	...
	xmlhttp.onreadystatechange = function () {
		if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
			...
			for (i = 0; i < arrayLength; i++) {
				...
				var eventsSkills = document.createElement('input');
				eventsSkills.setAttribute("id", "eventSkills" + x);
				eventsSkills.setAttribute("type", "hidden");
				eventBox.appendChild(eventsSkills);
				...
				if (!partOfArray(events[i].Event_id, AUXLIB.studentGoingToEvents())) {
					...
					input3.setAttribute("value", "learningEvents");
					form.appendChild(input3);
					var input4 = document.createElement('input');
					input4.setAttribute("type", "hidden");
					input4.setAttribute("id", "checkedCareergoals" + x);
					input4.setAttribute("name", "checkedCareergoals");
					input4.setAttribute("value", selectedCareerGoals.toString());
	...
	xmlhttp.open("GET", "./model/ajax/learningEventsFetch.php", true);
	xmlhttp.send();
}
    ...
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
			...
			for (i = 0; i < arrayLength; i++) {
				...
				document.getElementById('organiser' + x).innerHTML = organisationName;
				...
				var eventSkills = document.getElementById('eventSkills' + x);
				...
				for (j = 0; j < skills.length; j++) {
					...
					if (eventSkills.value == "null" || eventSkills.value == "") {
						eventSkills.value = skillId;
					} else {
						eventSkills.value = eventSkills.value + "," + skillId;
					}
				}
...
function displayEvents() {
	/* Empty the 'skillsToDisplay' array to refresh the whole array based upon the user input. */
	skillsToDisplay = [];
	var i;
	/* For every element in the 'selectedCareerGoals' array, place the skills of the sub-career-goals in the 'skillsToDisplay' array. */
	for (i = 0; i < careerGoalSkills.length; i++) {
		if (partOfArray(careerGoalSkills[i].parentId, selectedCareerGoals)) {
			var j;
			for (j = 0; j < careerGoalSkills[i].skills.length; j++) {
				/* If the skill is not a part of the 'skillsToDisplay' array yet, then place it in the array. */
				if (!partOfArray(careerGoalSkills[i].skills[j], skillsToDisplay)) {
					skillsToDisplay.push(careerGoalSkills[i].skills[j]);
				}
			}
		}
	}
	...
	for (i = 0; i < eventBoxes.length; i++) {
		...
		var eventSkills = document.getElementById('eventSkills' + x).value;
		eventSkills = eventSkills.split(',');
		...
		if (selectedCareerGoals[0] != null && selectedRegions[0] == null) {
			var j;
			for (j = 0; j < eventSkills.length; j++) {
				if (typeof skillsToDisplay != 'undefined') {
					if (partOfArray(eventSkills[j], skillsToDisplay)) {
						displayEvent = true;
					}
				}
			}
		} else if (selectedCareerGoals[0] != null && selectedRegions[0] != null) {
			var j;
			for (j = 0; j < eventSkills.length; j++) {
				if (typeof skillsToDisplay != 'undefined') {
					if (partOfArray(eventSkills[j], skillsToDisplay)) {
...
function processSelection(checkboxElem, selectionType, selectedValue) {
	...
	if (selectionType == "careerGoal") {
		var selection = document.getElementById('checkedCareergoals');
		if (checkboxElem.checked) {
			selectedCareerGoals.push(selectedValue);
			...
			var index = selectedCareerGoals.indexOf(selectedValue);
			selectedCareerGoals.splice(index,1);
		...
		for (i = 0; i < eventBoxes.length; i++) {
			var x = i+1;
			var checked = document.getElementById('checkedCareergoals' + x);
			if (checked != null) {
