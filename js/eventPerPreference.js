...
function loadCareerGoals() {
    ...
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
            ...
            for (i = 0; i < arrayLength; i++) {
				...
				checkBox.setAttribute("onchange", "passSelection(this," + parentId + ")");
            ...
            save.setAttribute("value", "Save these settings");
	...
	if (typeof AUXLIB.noSelection() === 'undefined') {
		showEvents(AUXLIB.passedCareerGoals(), AUXLIB.studentId());
	}
}


/* This function places the selected values in the hidden input field and then submits the form to load the corresponding events. */
function passSelection(checkboxElem, parentId) {
    var selection = document.getElementById('checked');
    if (checkboxElem.checked) {
        if (selection.value == "null" || selection.value == "") {
            selection.value = parentId;
        } else {
            selection.value = selection.value + "," + parentId;
        }
		document.getElementById('reload').click();
    } else {
        var array = selection.value.split(",");
		
		var index = array.indexOf(parentId.toString());
...
It takes 3 arguments: the id of the container it should create elements in, the list of selected career goals and the student id. */
function showEvents(careerGoals, studentId) {
    'use strict';
	...
	xmlhttp.onreadystatechange = function () {
		if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
			...
			if (arrayLength == 0) {
				eventList.innerHTML = "<p class='darkWatermark'>It looks like there are no upcoming events where you can work on this career goal.</p>";
			} else {
				for (i = 0; i < arrayLength; i++) {
					...
					if (!partOfArray(events[i].Event_id, AUXLIB.studentGoingToEvents())) {
						...
						input3.setAttribute("value", "eventPerPreference");
						form.appendChild(input3);
						var input4 = document.createElement('input');
						input4.setAttribute("type", "hidden");
						input4.setAttribute("name", "checked");
						input4.setAttribute("value", careerGoals);
	...
	xmlhttp.open("GET", "./model/ajax/eventListFetch.php?careerGoals=" + careerGoals, true);
	xmlhttp.send();
}
