...
function loadMembers(organisationId, userId) {
	...
	xmlhttp.onreadystatechange = function () {
		if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
			...
			var organisationAdminId = members[0];
			...
			for (i = 1; i < arrayLength; i++) {
				...
				if (memberId == organisationAdminId) {
				...
				if ((userId == organisationAdminId) && (userId != memberId)) {
					...
					form.appendChild(input1);var input2 = document.createElement('input');
					...
					form.appendChild(input3);var input4 = document.createElement('input');
					input4.setAttribute("type", "hidden");
					input4.setAttribute("name", "action");
					input4.setAttribute("value", "eventOrganisation");
	...
	xmlhttp.open("GET", "./model/ajax/organisationMembersFetch.php?organisationId=" + organisationId, true);
	xmlhttp.send();
}
