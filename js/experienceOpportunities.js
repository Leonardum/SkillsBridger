var jobTypeArray = [];
if (AUXLIB.passedJobTypes() != undefined) {
	var selectedJobTypes = AUXLIB.passedJobTypes().split(',');
} else {
...
function loadJobTypes() {
    ...
    var jobTypes = document.getElementById('jobTypes');
    
	var instruction = document.createElement('p');
	instruction.innerHTML = "Refine your search:";
	jobTypes.appendChild(instruction);
	
	var instructionTitle = document.createElement('p');
	instructionTitle.setAttribute("class", "instructionTitle");
	instructionTitle.innerHTML = "By job type:";
	jobTypes.appendChild(instructionTitle);
	
	var form = document.createElement('form');
	form.setAttribute("method", "post");
	form.setAttribute("action", AUXLIB.thisPage());
	jobTypes.appendChild(form);
	
	jobTypeArray = ["internship", "half-time job", "full-time job"];
	var i;
	var arrayLength = jobTypeArray.length;
	for (i = 0; i < arrayLength; i++) {
		var jobType = jobTypeArray[i];
		
		var checkBox = document.createElement('input');
		checkBox.setAttribute("type", "checkbox");
		checkBox.setAttribute("id", jobType.toLowerCase().replace(/\s+/g, '') + "Box");
		if (typeof AUXLIB.checkList() !== 'undefined') {
			if (partOfArray(jobType, AUXLIB.checkList())) {
				checkBox.checked = true;
			}
		}
		checkBox.setAttribute("onchange", "processSelection(this, 'eventType', '" + jobType + "')");
		form.appendChild(checkBox);
		
		var span = document.createElement('span');
		span.setAttribute("class", "tickBoxText");
		span.innerHTML = jobType.capitalize();
	...
	var checkedJobTypes = document.createElement('input');
	checkedJobTypes.setAttribute("type", "hidden");
	checkedJobTypes.setAttribute("id", "checkedJobTypes");
	checkedJobTypes.setAttribute("name", "checkedJobTypes");
	if (typeof AUXLIB.passedJobTypes() !== 'undefined') {
		checkedJobTypes.setAttribute("value", AUXLIB.passedJobTypes());
	} else {
		checkedJobTypes.setAttribute("value", "");
	}
	form.appendChild(checkedJobTypes);
...
function loadRegions() {
    ...
    regionsRequest.open("GET", "./model/ajax/expOpportunitiesFetch.php?regionsLoad=true", true);
    regionsRequest.send();
}
...
function loadEvents() {
	...
	xmlhttp.onreadystatechange = function () {
		if (xmlhttp.readyState === 4 && xmlhttp.status === 200) {
			...
			for (i = 0; i < arrayLength; i++) {
				...
				eventTypeInput.value = events[i].Type;
				...
				if (!partOfArray(events[i].Event_id, AUXLIB.studentGoingToEvents())) {
					...
					input3.setAttribute("value", "experienceOpportunities");
					form.appendChild(input3);
					var input4 = document.createElement('input');
					input4.setAttribute("type", "hidden");
					input4.setAttribute("id", "checkedJobTypes" + x);
					input4.setAttribute("name", "checkedJobTypes");
					input4.setAttribute("value", selectedJobTypes.toString());
	...
	xmlhttp.open("GET", "./model/ajax/expOpportunitiesFetch.php", true);
	xmlhttp.send();
}
...
function displayEvents() {
	...
	for (i = 0; i < eventBoxes.length; i++) {
		...
		if (selectedJobTypes[0] != null && selectedRegions[0] == null) {
			if (partOfArray(eventType, selectedJobTypes)) {
				displayEvent = true;
			} else {
				displayEvent = false;
			}
		} else if (selectedJobTypes[0] != null && selectedRegions[0] != null) {
			if (partOfArray(eventType, selectedJobTypes)) {
				displayEvent = true;
			}
	...
	if (eventsDisplayed == false) {
		...
		watermark.innerHTML = "It looks like there are no upcoming experience opportunities matching your selection.";
	} else {
		eventList.className = "col-16 masonry sidebarContent";
	}
}
...
function processSelection(checkboxElem, selectionType, selectedValue) {
	...
	if (selectionType == "eventType") {
		var selection = document.getElementById('checkedJobTypes');
		if (checkboxElem.checked) {
			selectedJobTypes.push(selectedValue);
			...
			var index = selectedJobTypes.indexOf(selectedValue);
		...
		for (i = 0; i < eventBoxes.length; i++) {
			var x = i+1;
			var checked = document.getElementById('checkedJobTypes' + x);
			if (checked != null) {
